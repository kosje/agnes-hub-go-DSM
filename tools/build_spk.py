...
#
# 图标源是 assets/ICON.PNG —— 一幅「铬质 A 字 + 上方圆点 + 星空山景」的插画
# （413×384，RGB 无 alpha）。
#
# 注意：原始素材在 A 字外圈还有一道细白圆环（半径 192px、圆心 (208,209)），
# 在 64px 的桌面图标上会读成一个「加载中」的转圈，用户明确要求去掉，现已
# 从源图里抹除。抹除用的是「按半径插值 + 带内高斯模糊」，做法与脚本一并留在
# tools/remove_icon_ring.py，换素材后需要重跑。
#
# 它不是扁平 logo，所以没法靠阈值把字形抠出来重绘：背景里有明亮的光柱、光环和
# 山体，实测都会跟字形连成一片，连通域分析拿不到干净的字形包围盒。所以这里改成
# 整幅裁切，思路是：
#   · 按实测的视觉重心裁一个正方形，把「A + 圆点」框进来；
#   · 缩放在线性光空间做面积平均 —— 直接在 sRGB 上平均会把暗部抬亮、亮部压暗，
#     缩小后就是一张发灰的糊图（这是照片类图标最容易翻车的地方）；
#   · 轻微提对比、提饱和、加一圈暗角，让图标读起来是一块完整的「瓦片」；
#   · 套 DSM 的圆角遮罩（当前 ICON_CORNER_RADIUS=0.5，即正圆），
#     小尺寸再往里收一点，保证「A」还认得出来。
# ---------------------------------------------------------------------------
...
# 图标外形：圆角矩形的圆角比例（按图标边长）。
#
# 0.5 = 正圆 —— 用户明确要求圆形图标（A 字与插画内容保持不变，只改外轮廓）。
# 0.22 是群晖自家套件常见的圆角方形比例，留作参考。
# 注意半径取到 0.5 时 _rrect_span 退化成标准圆：圆心在正中、直径等于边长，
# 四角被完全切掉，所以构图必须保证主体落在内切圆里 —— 当前源图的 A 字满足。
ICON_CORNER_RADIUS = 0.5

# 小尺寸下额外往里收的倍率（按图标边长）。16/24px 里整幅插画会糊成一团，
# 收一点让「A」占到更多像素，至少还认得出是个字母。
#
# 72 这一档是补上的：DSM 桌面与主菜单取的正是 64/72，而原先只做到 48，
# 于是桌面图标成了唯一没享受「往里收」的一档 —— 加上暗角就是一团糊。
ICON_SMALL_ZOOM = ((32, 1.14), (48, 1.07), (72, 1.05))
