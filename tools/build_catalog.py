...
CHANGELOG = (
    "【1.0.17】<br>"
    "· 重排控制台「视频任务」页：长 job_id/video_id 收成「前 8…后 4」并支持点击复制，"
    "表格不再横向溢出、列不再挤到折行<br>"
    "· 「承载账号」直接显示账号名（原来只有一串 acc_ 开头的 ID）<br>"
    "· 新增「补取」按钮：本地落盘功能上线前完成的任务没有本地副本，"
    "点一下即可回上游取回并落盘<br>"
    "· 修正「播放」按钮的判断 —— 1.0.16 起地址改为绝对地址，原判断会误报「没有本地副本」<br>"
    "<br>"
    "【1.0.16】<br>"
    "· 【重要】修复在 AI 客户端里生图「不出图、只显示一串莫名其妙的文字」："
    "提示词被取成客户端自己的框架块（&lt;craft_mode&gt; 等），且图片地址是相对路径客户端无法解析<br>"
    "· 生成的图片/视频改为返回绝对地址；新增「对外访问地址」设置项<br>"
    "· 媒体回放接口改为免鉴权（文件名是内容的 128 位指纹）<br>"
    "<br>"
    "【1.0.15】<br>"
