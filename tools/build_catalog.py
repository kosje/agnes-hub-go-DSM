...
CHANGELOG = (
    "【1.0.14】<br>"
    "· 【重要】修复生视频一直报 400「mode is required」：Agnes Video 2.5 把 mode 列为必填，"
    "而网关的字段白名单里根本没有它<br>"
    "· 【重要】视频请求体改为按模型家族分两套口径：2.5 用 mode/seconds/size/aspect_ratio，"
    "v2.0 用 width/height/num_frames/frame_rate —— 两代参数互不兼容，混用必 400<br>"
    "· 修复 JSON 数组型字段被静默丢弃：判空用的是只认标量的转换函数，"
    "导致 \"images\": [\"https://...\"] 这类参考素材写法发不出去（生图同样受影响）<br>"
    "· 请求体按最终选中的模型重建，避免「换模型名不换参数」导致跨家族 400<br>"
    "· 生视频探测（账号测试）同步改用统一口径<br>"
    "<br>"
    "【1.0.13】<br>"
    ...
    "· 上游错误信息不再只剩一个 HTTP 400：兼容 message/msg/detail/error 等字段名<br>"
    "· 新增控制台「向上游拉取可用模型」：拿账号 Key 问上游 /v1/models，"
    "直接标出本地清单里填了但上游没给的模型<br>"
    "· 控制台「查看全部版本」改为 GitHub 徽标链接<br>"
