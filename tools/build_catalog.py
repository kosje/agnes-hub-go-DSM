...
CHANGELOG = (
    "【1.0.22】<br>"
    "· 修 1.0.21 没修好的问题：地址行改写成「行内代码」—— 客户端会把链接的显示文字"
    "换成「域名+路径」（吃掉 https:// 与端口），连显式链接文字也照改；"
    "行内代码不会被当成链接处理，才原样显示<br>"
    "· 同时保留一个「在浏览器中打开」的可点入口<br>"
    "<br>"
    "【1.0.21】<br>"
    "· 图片下面的地址行去掉 ?download=1 —— 点开即在浏览器里看图，不再强制存盘"
    "（服务端仍认这个参数，需要下载时自己补上）<br>"
    "· 地址行改为「链接文字就是完整地址」的 Markdown 链接：客户端会把裸 URL 自动链接"
    "并美化掉 https:// 与端口，显示出来跟真实地址对不上；显式链接文字不会被改写<br>"
    "<br>"
    "【1.0.20】<br>"
    "· 套件图标外轮廓由圆角方形改成正圆（插画与 A 字图形完全不变），"
    "Web UI 顶栏 logo 同步变圆<br>"
    "· 回复正文里不再追加「字段 xxx 未被生图端点接受，已忽略」那一行 —— "
    "它会跟着图片沉进对话记录；改由响应头 X-Agnes-Hub-Dropped-Fields 带回<br>"
    "<br>"
    "【1.0.19】<br>"
    "· 【重要】客户端媒体地址改为三档，默认「自动」：配了「对外访问地址」就用网关地址，"
    "没填才退回上游。升级时自动迁移「upstream + 已配对外地址」的组合，无需手动改设置<br>"
    "· 【重要】修复自检面板骗人：它拿网页请求去推算「客户端将收到」，被网页标记短路，"
    "不管怎么配都显示网关地址<br>"
    "· 【重要】套件图标去掉 A 字外圈的细白圆环（64px 桌面图标上会读成「加载中」转圈）；"
    "顺带补上 64/72 档的取景补偿<br>"
    "· 生成的图片在正文里额外给出一条可复制的「原图下载」地址（?download=1，点开即存盘）<br>"
    "<br>"
    "【1.0.18】<br>"
    "· 【重要】修复外部 AI 客户端里生图「不出图、只显示一串文字」："
    "本地媒体地址客户端加载不了（它在别的网络位置、甚至跑在 https 页面里）。"
    "现在按调用方分流 —— 网关自己的网页用本地副本，外部客户端默认用上游地址<br>"
    "· 新增「客户端媒体地址」设置：已把网关反代到公网 HTTPS 的，"
    "改成「本机地址」即可让客户端也看本地副本（离线可看）<br>"
    "· 新增「查看当前生效的媒体地址」自检按钮：把按当前请求推算出的基地址摊开，"
    "专门用来排查反向代理少传端口的问题<br>"
    "· 视频任务记录新增 source_url（上游原始地址），两个地址都留着才选得出该给谁<br>"
    "<br>"
    "【1.0.17】<br>"
    "· 重排控制台「视频任务」页：长 ID 收成「前 8…后 4」并支持点击复制，"
    "表格不再横向溢出<br>"
    "· 「承载账号」直接显示账号名；新增「补取」按钮取回旧任务的产出<br>"
    "<br>"
    "【1.0.16】<br>"
    "· 【重要】修复提示词被取成客户端自己的框架块（&lt;craft_mode&gt; 等），"
    "用户原话根本没发给上游<br>"
    "· 生成的图片/视频改为返回绝对地址；媒体回放接口改为免鉴权<br>"
    "<br>"
    "【1.0.15】<br>"
    "· 生成的视频改为落盘保存，聊天窗口与「视频任务」页播放本地副本<br>"
    "· 视频缓存按「条数 + 总字节」双重淘汰，默认上限 2 GB<br>"
    "<br>"
    "【1.0.14】<br>"
    "· 【重要】修复生视频一直报 400「mode is required」<br>"
    "· 【重要】视频请求体按模型家族分两套口径（2.5 与 v2.0 参数互不兼容）<br>"
    "· 修复 JSON 数组型字段被静默丢弃（\"images\": [\"https://...\"] 发不出去）<br>"
    "<br>"
    "【1.0.13】<br>"
    "· 版本号规则改为纯 x.y.z：发新版直接 +1，不再带 -0001 构建号<br>"
    "· 生成的图片改为落盘保存<br>"
    "· 【重要】修复生视频链路：提交返回本地 job_id；查询改走上游真正的查询接口<br>"
    "· 上游错误信息不再只剩一个 HTTP 400<br>"
    "· 新增控制台「向上游拉取可用模型」；「查看全部版本」改为 GitHub 徽标链接<br>"
    "· 清理旧品牌标识（升级后需重新登录一次控制台）<br>"
    "<br>"
    "【1.0.12-0002 / 1.0.12-0001】<br>"
    "· 【重要】修复生图打不通（/api/chat 前缀没摘）与 agnes-auto 不被识别为 auto<br>"
    "· 控制台品牌改为 Agnes Hub；修复网页顶栏 logo 升级后不更新"
)
