...
CHANGELOG = (
    "重做 DSM 图标，彻底解决「套件图标显示不对」的问题。<br>"
    "· 图标改为按矢量精度重绘：从上游 logo 里量出白色字形的真实几何（外框、边框厚度、分隔线位置），再按几何重新栅格化，16/24/32/48/64/72/128/256 全尺寸都清晰、无锯齿<br>"
    "· 去掉原图的黑底与浅紫圆盘：原 logo 是「黑底 + 浅紫圆盘 + 白字形」的 RGB 图，抠掉黑底后白字形压在浅紫圆盘上对比度极低、还会与方底形成两层轮廓。现在改为品牌蓝渐变圆角方底 + 白色字形 + 淡蓝三格，浅色/深色主题下都清晰<br>"
    "· 同步上游 1.0.11：修复 AMD 空响应体、软粘性会话绑定模型判定，新增账号编辑完整弹窗与优先级支持"
)
