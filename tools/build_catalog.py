...
CHANGELOG = (
    "【1.0.15】<br>"
    "· 生成的视频改为落盘保存，聊天窗口与「视频任务」页都播放本地副本，"
    "不再依赖浏览器能否访问上游 CDN<br>"
    "· 视频缓存按「条数 + 总字节」双重淘汰，默认上限 2 GB，可在设置里调整<br>"
    "· 视频回放支持 Range 请求，进度条可拖动、可断点续传<br>"
    "· 图片与视频的落盘逻辑合并为一套通用实现，各自的上限/超时/扩展名口径独立<br>"
    "<br>"
    "【1.0.14】<br>"
    "· 【重要】修复生视频一直报 400「mode is required」：Agnes Video 2.5 把 mode 列为必填，"
    "而网关的字段白名单里根本没有它<br>"
    "· 【重要】视频请求体按模型家族分两套口径：2.5 用 mode/seconds/size/aspect_ratio，"
