...
CHANGELOG = (
    "【1.0.16】<br>"
    "· 【重要】修复在 AI 客户端里生图「不出图、只显示一串莫名其妙的文字」：<br>"
    "  · 提示词取错 —— 客户端会在对话尾部追加自己的框架块（&lt;craft_mode&gt; 等），"
    "它们同样是 role=user，网关把框架指令当成了用户的话，用户原话根本没发给上游<br>"
    "  · 图片地址是相对路径 —— 只有网关自己的网页能解析，外部客户端必然加载失败，"
    "只能退化成显示图片的 alt 文本<br>"
    "· 生成的图片/视频改为返回绝对地址；新增「对外访问地址」设置项"
    "（留空按请求 Host 自动推断，走反向代理时建议显式填写）<br>"
    "· 媒体回放接口改为免鉴权（文件名是内容的 128 位指纹，属 capability URL）—— "
    "外部客户端不可能带上网关的会话 cookie，要求登录就等于永远裂图<br>"
    "<br>"
    "【1.0.15】<br>"
    "· 生成的视频改为落盘保存，聊天窗口与「视频任务」页播放本地副本<br>"
    "· 视频缓存按「条数 + 总字节」双重淘汰，默认上限 2 GB<br>"
    "· 视频回放支持 Range 请求，进度条可拖动、可断点续传<br>"
    "<br>"
    "【1.0.14】<br>"
