...
1. **新增服务端「base64 回取代理」：生成的图片现在能真的显示出来**（本次最重要的修复）：

   - 之前生图虽然成功了，但正文里写的是一行 `![提示词](https://上游地址)`，**浏览器加载不出来** —— 上游返回的图片地址往往带鉴权、或落在浏览器够不到的 CDN 上，直接交给 `<img>` 只会渲染成一张裂图。
   - 现在由**服务端把图片拉回来**，转成 `data:image/...;base64,...` 内联进正文，对话窗口按 Markdown 直接显示，零依赖、不受跨域 / 鉴权影响。
   - 对话页的「AI 生图」标签页（走官方 `data[].url` 形态）同样已内联，`<img src>` 直接可用。
   - **裸 `/v1` 接口保持原样**：程序化客户端（curl / 官方 SDK）拿到的仍是真实的上游地址，不会被塞进几 MB 的 base64。这条边界是刻意划的 —— 网页要能看，程序要能自己下载。
   - 回取过程全程保守：非 http(s)、已是 data URI、上游非 200、响应不是图片（例如 WAF 的 HTML 拦截页）、超过 6 MB —— 任一条件命中就原样退回上游地址，**宁可退化成裂图，也不会让整个响应挂掉**。
   - 对话记录入库前会把这段 base64 压成一行提示：单张图 1~3 MB，原样存进 JSON 会瞬间把存储撑爆（几十条记录就能上百 MB）。

2. **控制台「版本与自更新」面板修好**：

   - 套件版（由套件中心统一升级）**仍然可以查到最新版本号与发布时间**。之前把「能不能查」和「能不能装」合成了一个开关，导致套件版这两格永远只显示「—」；
   - 「发布时间」精确到分钟（服务器本地时区），方便判断这次检查拿到的到底是不是刚发的版本；
   - 「查看全部版本」只保留一个 GitHub 地址链接，并跟着后端实际使用的仓库走，不会指向一个没有 SPK 的地方。

3. 顺手清理：控制台退出日志与 SSE 心跳注释里的旧品牌字样。

---

### 上一版（1.0.12-0001）

1. **修复「生图 / 生视频」整条链路打不通**（共四处缺陷）：
...
3. **修复网页顶栏图标升级后不更新**：
   ...
   - `/logo.png` 原先带 `Cache-Control: public, max-age=86400`（缓存一整天），而 logo 是 `go:embed` 进二进制的、URL 恒定不变 —— 换图标后浏览器会一直用缓存里的旧图。
   - 改为 `no-cache` + 内容指纹 `ETag`（内容没变就回 304），并让前端引用地址带上指纹（`/logo.png?v=<指纹>`）。指纹一变 URL 就变，缓存必然击穿。
...
- **x86_64 机型**：[agnes-hub-x86_64-1.0.12-0002.spk](https://kosje.github.io/agnes-hub-go-DSM/agnes-hub-x86_64-1.0.12-0002.spk)
- **armv8 机型**：[agnes-hub-armv8-1.0.12-0002.spk](https://kosje.github.io/agnes-hub-go-DSM/agnes-hub-armv8-1.0.12-0002.spk)

---

### 升级后图标 / 页面还是旧的？

1. 在套件中心确认版本已是 `1.0.12-0002`；
...
> 说明：`1.0.11-0003` 及更早版本里，顶栏 logo 被 `max-age=86400` 缓存住，升级后最长要等 24 小时才会自己变过来 —— 这是 `1.0.12-0001` 修复的第 3 条。
